...
export  default  function Navbar() {
 ...
 const countContext = useContext(CounterContext);
  ...
  return (
    <div className="w-3/4 relative bg-[#eaf1f1] my-2 mx-auto flex justify-between items-center p-2 rounded-full">
      {/* Logo */}
        ...
        <div className="w-1/4">
        <Link href="/">
         <div className=" flex items-center justify-center text-sm font-bold">BUY <span className="text-xl text-cyan-700">HIVE</span></div>
        </Link>
        </div>
  ...
  <DropdownMenu >
    ...
    <DropdownMenuTrigger className={`${navigationMenuTriggerStyle()} cursor-pointer `}>
        <Image src='/icons/user.png' alt='cartIcon' width={20} height={20} className="w-full"/>
    ...
    <DropdownMenuContent className="w-40 bg-white">
      ...
      <DropdownMenuSeparator />
                 ...
                 <NavigationMenuItem>
                  ...
                  >
                   ...
                   <Image src='/icons/shopping-cart.png' alt='cartIcon' width={20} height={20} className="w-full"/>
                   <span className="absolute top-0 right-0">{countContext?.count}</span>
                  </Link>
                </NavigationMenuItem>
                 <NavigationMenuItem className="hidden lg:flex">
                  <button
                    onClick={() => signOut()}
                    className={`${navigationMenuTriggerStyle()} text-red-500 cursor-pointer `}
                  >
                   <Image src='/icons/log-out.png' alt='cartIcon' width={20} height={20} className="w-full"/>
                  </button>
                </NavigationMenuItem>
      ...
      {/* Mobile Menu */}
      {toggleBar && (
        <div className="absolute top-20 left-0 w-full bg-white shadow-md p-6 rounded-xl flex flex-col gap-4 lg:hidden z-50">
          {components.map((item, index) => (
            <Link
              key={index}
              href={item.Path}
              className={`p-2 rounded-md ${
                path === item.Path ? "bg-[#cdd2d3]" : "hover:bg-gray-100"
              }`}
          ...
          {status === "authenticated" ? (
            <>
              ...
              <button
                ...
                className="p-2 text-left text-red-500 rounded-md hover:bg-gray-100"
              >
                Logout
              </button>
            </>
